...
function Map({
  ...
  const semaphoreOptions = ['VERDE', 'VERMELHO', 'AMARELO']
  ...
  function styleLine(feature: Feature<Geometry, any>) {
    const enterprise = initialEnterprises.enterprises.find((item) => {
      return +item.id === feature.properties.ID
    })

    const semaphore = enterprise.semaphore as 'VERDE' | 'AMARELO' | 'VERMELHO'

    if (semaphoreOptions.includes(semaphore)) {
      return {
        color: semaphoreColor[semaphore],
        weight: 4
      }
    }
    return { color: '#0c326f', weight: 2 }
  }
  ...
  useEffect(() => {
    ...
    const pointsLayer = L.geoJSON(
      ...
      {
        style: function (feature) {
          return styleLine(feature as Feature<Geometry, any>)
        }
      }
    ).addTo(mapRef.current)
